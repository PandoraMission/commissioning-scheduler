...
class Observation:
    ...
    def __post_init__(self):
        ...
        if self.boresight_ra is not None and not (
            0 <= self.boresight_ra <= 360
        ):
            raise ValueError(
                f"RA must be between 0 and 360, got {self.boresight_ra}"
            )
        if self.boresight_dec is not None and not (
            -90 <= self.boresight_dec <= 90
        ):
            raise ValueError(
                f"DEC must be between -90 and 90, got {self.boresight_dec}"
            )

        # Validate priority if provided
        if self.priority is not None and not (0 <= self.priority <= 10):
            raise ValueError(
                f"Priority must be between 0 and 10, got {self.priority}"
            )
...
class Visit:
    ...
    visit_id: str
    observation_sequences: List[ObservationSequence] = field(
    ...
    def total_science_minutes(self) -> float:
        ...
        return sum(
            seq.science_duration_minutes or 0.0
...
class MoonshineConfig:
    ...
    def __post_init__(self):
        ...
        # If both are specified, raise error
        if (
...
class SchedulerConfig:
    ...
    # Constraints
    keep_out_angles: Tuple[float, float, float] = (
        90.0,
        25.0,
        63.0,
    )  # sun, earth, moon
    max_sequence_duration_minutes: float = (
        90.0  # Maximum continuous observation time
    )
    ...
    cache_dir: Optional[str] = (
        None  # If None, will use default hidden directory
    )
    keep_raw_xml_trees: bool = True  # Keep XML trees for cloning
    verify_cvz_visibility: bool = (
        False  # If False, skip CVZ visibility check (faster)
    )
    ...
    def __post_init__(self):
        ...
        if self.commissioning_start >= self.commissioning_end:
            raise ValueError(
                "commissioning_start must be before commissioning_end"
            )
        if len(self.keep_out_angles) != 3:
            raise ValueError(
                "keep_out_angles must have 3 values (sun, earth, moon)"
            )
...
class SchedulingResult:
    ...
    scheduled_sequences: List[ObservationSequence] = field(
        default_factory=list
    )  # ADD THIS
...
class ContinuousObservationConstraint:
    ...
    def applies_to(self, obs: Observation) -> bool:
        ...
        return (obs.task_number in self.tasks) or (
