...
class ObservationParser:
    ...
    TASK_FILENAME_RE = re.compile(r"(?P<task>\d{4})_(?P<seq>\d{3})_.*\.xml$", re.IGNORECASE)
    ...
    def parse_file(self, filename: str) -> Observation:
        ...
        if seq_elem is None:
            raise RuntimeError(f"No <Observation_Sequence> element found in {filename}")
        ...
        if vis_block is not None:
            ...
            if ntfr:
                try:
                    ...
                    logger.warning(f"Could not parse NumTotalFramesRequested: {ntfr}")
            ...
            if obs.num_total_frames_requested and obs.exposure_time_us:
                total_seconds = (obs.num_total_frames_requested * obs.exposure_time_us) / 1e6
                obs.visible_duration = total_seconds / 60.0
                logger.debug(f"Calculated visible duration: {obs.visible_duration:.3f} minutes")
        ...
        if nir_block is not None:
            ...
            if obs.sc_integrations and obs.roi_sizex and obs.roi_sizey:
                integration_time_sec = self._calculate_nir_integration_time(obs)
                total_seconds = obs.sc_integrations * integration_time_sec
                obs.nir_duration = total_seconds / 60.0
                logger.debug(f"Calculated NIR duration: {obs.nir_duration:.3f} minutes")
        ...
        if obs.duration is None:
            ...
            if nir > 0 or vis > 0:
                ...
                logger.debug(f"Manually calculated total duration: {obs.duration:.3f} minutes")
            else:
                logger.warning(f"Could not calculate duration for {filename} - no camera data found")
...
def gather_task_xmls(xml_dir: str, pattern: str = r"^\d{4}_\d{3}_.*\.xml$") -> List[str]:
    """
...
class ScheduleWriter:
    ...
    def write_schedule(self, visits: List[Visit], output_path: str, 
        ...
        if metadata:
            ...
            # Default metadata
            meta.set('Valid_From', format_utc_time(self.config.commissioning_start))
            meta.set('Expires', format_utc_time(self.config.commissioning_end))
            meta.set('Calendar_Weights', '0.0, 0.0, 1.0')
            meta.set('Created', format_utc_time(datetime.now()))
        
        # Sort ALL visits by their start time (chronological order)
        sorted_visits = sorted(visits, key=lambda v: v.start_time if v.start_time else datetime.max)
        
        logger.info(f"Writing {len(sorted_visits)} visits in chronological order")
        ...
        logger.info(f"Wrote schedule with {len(visits)} visits to {output_path}")
    
    def _create_visit_element(self, visit: Visit, visit_num: int) -> ET.Element:
        """
        ...
        for seq in visit.observation_sequences:
            seq_elem = self._create_observation_sequence_element(seq, visit.visit_id)
            visit_elem.append(seq_elem)
        
        return visit_elem
    
    def _create_observation_sequence_element(self, seq: ObservationSequence, task_id: str) -> ET.Element:
        """
        ...
        elif seq.parent_observation and seq.parent_observation.raw_xml_tree is not None:
            elem = self._clone_and_adjust_sequence(seq, task_id)
        else:
    ...
    def _clone_and_adjust_sequence(self, seq: ObservationSequence, task_id: str) -> ET.Element:
        """
        ...
        if task_elem is None:
            ...
            id_index = list(seq_elem).index(id_elem) if id_elem is not None else 0
            ...
            if 'NumTotalFramesRequested' in adjusted:
                ...
                if vis_elem is not None:
                    ...
                    if ntfr_elem is not None:
                        ntfr_elem.text = str(adjusted['NumTotalFramesRequested'])
    ...
    def _create_minimal_sequence(self, seq: ObservationSequence, task_id: str) -> ET.Element:
        """
        ...
        if seq.parent_observation:
            ...
            dec = dec if dec is not None else seq.parent_observation.boresight_dec
            priority = priority if priority is not None else seq.parent_observation.priority
    ...
    def _needs_vis_block(self, elem: ET.Element, seq: ObservationSequence) -> bool:
    ...
    def _ensure_vis_camera_block(self, elem: ET.Element, seq: ObservationSequence):
        ...
        ET.SubElement(vis_block, 'TargetRA').text = str(seq.boresight_ra or 0.0)
        ET.SubElement(vis_block, 'TargetDEC').text = str(seq.boresight_dec or 0.0)
        ...
        logger.debug(f"Added visible camera block to {seq.obs_id}_{seq.sequence_id}")
    ...
    def write_schedule_from_sequences(
        ...
        logger.info(f"Grouped {len(sequences)} sequences into {len(visits)} visits")
        
        # Write visits with sequential IDs
        for visit_num, visit in enumerate(visits, start=1):
            visit_elem = self._create_visit_element_from_sequences(visit, visit_num)
            root.append(visit_elem)
    ...
    def _group_sequences_into_visits(self, sequences: List[ObservationSequence]) -> List[List[ObservationSequence]]:
        """
        ...
        for i in range(1, len(sequences)):
            ...
            both_cvz = (curr_seq.obs_id in ["CVZ", "CVZ_BLOCKED"] and 
                    prev_seq.obs_id in ["CVZ", "CVZ_BLOCKED"])
            same_cvz_type = (curr_seq.obs_id == prev_seq.obs_id and 
                            curr_seq.obs_id in ["CVZ", "CVZ_BLOCKED"])
            
            # Time gap check
            time_gap_minutes = (curr_seq.start - prev_seq.stop).total_seconds() / 60.0
            is_consecutive = time_gap_minutes < 1.0
            
            # Special case for task 0312: keep all sequences together even if not consecutive
            is_task_0312 = (curr_seq.obs_id == "0312_000" and prev_seq.obs_id == "0312_000")
    ...
    def _create_staring_sequence(self, seq: ObservationSequence, task_id: str) -> ET.Element:
        """
        ...
        ET.SubElement(vis_block, 'RiceY').text = '25'
        ET.SubElement(vis_block, 'SendThumbnails').text = '0'
        ET.SubElement(vis_block, 'TargetID').text = seq.target_name or 'Unknown'
        ET.SubElement(vis_block, 'TargetRA').text = str(seq.boresight_ra or 0.0)
        ET.SubElement(vis_block, 'TargetDEC').text = str(seq.boresight_dec or 0.0)
        ET.SubElement(vis_block, 'StarRoiDetMethod').text = '3'
        ET.SubElement(vis_block, 'numPredefinedStarRois').text = '1'
