...
try:
    ...
    logger.warning("pandoravisibility not available - visibility calculations will fail")
...
def compute_antisolar_coordinates(time: Time, dec: float = 8.5) -> Tuple[float, float]:
    """
...
class VisibilityCalculator:
    ...
    def __init__(
        ...
        logger.info(f"Creating time grid: {len(self.times)} points from {start.iso} to {stop.iso}")
    ...
    def _get_spacecraft_state(self):
        ...
        if self._state_cache is None:
            logger.info("Computing spacecraft state (this may take a moment)...")
    ...
    def compute_for_observations(
        ...
        for obs in observations:
            if obs.boresight_ra is None or obs.boresight_dec is None:
                logger.warning(f"Observation {obs.obs_id} missing coordinates, skipping visibility")
                continue
            key = (float(obs.boresight_ra), float(obs.boresight_dec))
            target_map.setdefault(key, []).append(obs)
        
        logger.info(f"Computing visibility for {len(target_map)} unique targets")
        ...
        if parallel and len(target_map) > 1:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_radec = {
                    executor.submit(self._compute_for_target, ra, dec, force_recompute): (ra, dec)
                        ...
                        for obs in target_map[(ra, dec)]:
                            if attach_to_observations:
                                obs.visibility_windows = [
                                    (w.start.datetime, w.end.datetime) for w in vis_result.windows
                                ]
                            results[obs.obs_id] = vis_result
                    except Exception as e:
                        logger.error(f"Error computing visibility for ({ra}, {dec}): {e}")
        else:
            for (ra, dec), obs_list in target_map.items():
                try:
                    vis_result = self._compute_for_target(ra, dec, force_recompute)
                    for obs in obs_list:
                        if attach_to_observations:
                            obs.visibility_windows = [
                                (w.start.datetime, w.end.datetime) for w in vis_result.windows
    ...
    def _extract_windows(self, visible_bool: np.ndarray) -> List[VisibilityWindow]:
        """
    ...
    def _load_from_disk_cache(self, ra: float, dec: float) -> Optional[VisibilityResult]:
    ...
    def _save_to_disk_cache(self, ra: float, dec: float, result: VisibilityResult) -> None:
    ...
    def find_cvz_pointings_for_gaps(
        ...
        for gap_start, gap_end in gaps:
            ...
            if coords:
                ...
                antisolar_ra, fallback_dec = compute_antisolar_coordinates(midpoint, dec)
                logger.warning(
                    f"Using antisolar coordinates as fallback for gap "
                    f"{gap_start.iso} to {gap_end.iso}"
                )
                cvz_pointings[(gap_start, gap_end)] = (antisolar_ra, fallback_dec)
    ...
    def compute_visibility_for_window(
        ...
        if start_idx >= len(self.times) or end_idx == 0:
            logger.debug(f"Window outside precomputed range, computing visibility for window")
            # Create a small time grid just for this window
            n_steps = int((window_end - window_start).to_value('sec') / self.timestep_seconds)
...
def find_visible_cvz_pointing(
    ...
    if skip_visibility_check:
        logger.debug(f"Skipping CVZ visibility check, using antisolar RA={antisolar_ra:.2f}")
        return antisolar_ra, dec
    ...
    for i in range(max_iterations):
        ...
        is_visible = calc.compute_visibility_for_window(test_ra, dec, start_time, end_time)
...
def clear_visibility_cache(cache_dir: Optional[str] = None, confirm: bool = True) -> int:
    """
