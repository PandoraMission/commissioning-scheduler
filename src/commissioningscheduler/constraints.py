...
from models import (
    Observation, ObservationSequence, SchedulerConfig,
...
class ConstraintChecker:
    ...
    def check_sequence_valid(
        ...
        if previous_seq is not None:
            ...
            gap_minutes = (seq.start - previous_seq.stop).total_seconds() / 60.0
    ...
    def compute_slew_time(
        ...
        """
        separation = calculate_angular_separation(from_ra, from_dec, to_ra, to_dec)
    ...
    def split_observation_into_sequences(
        ...
        if self.requires_continuous_scheduling(obs):
            logger.info(f"    Observation {obs.obs_id} requires continuous scheduling")
            
            # Find a window that can fit the entire observation + overhead
            required_minutes = obs.duration
            overhead_minutes = self.config.slew_overhead_minutes if needs_initial_overhead else 0.0
            ...
            for win_start, win_end in visibility_windows:
                ...
                available_minutes = (win_end - win_start).total_seconds() / 60.0
        ...
        logger.debug(f"    Splitting {obs.obs_id}: duration={obs.duration:.4f} min, needs_overhead={needs_initial_overhead}")

        sequences = []
        remaining_minutes = obs.duration
        overhead_minutes = self.config.slew_overhead_minutes if needs_initial_overhead else 0.0
        overhead_to_apply = overhead_minutes  # Track if overhead still needs to be applied
        ...
        for win_start, win_end in visibility_windows:
            ...
            if last_seq_end > start_from:
                ...
                if gap_hours > max_gap_between_sequences_hours:
                    logger.warning(f"{obs.obs_id}: Gap too large ({gap_hours:.1f}h)")
                    break
            ...
            while remaining_minutes > 0 and current_pos < win_end:
                ...
                logger.debug(f"      Available at {current_pos}: {available:.4f} min")
                ...
                logger.debug(f"      Returning sequence: science={seq_science:.4f} min at {current_pos}")
                # Add sequence
                ...
                if overhead_to_apply > 0:
                    current_pos += timedelta(minutes=seq_science + overhead_to_apply)
                    overhead_to_apply = 0.0  # Only first sequence gets overhead
                else:
        ...
        if remaining_minutes > 0.01:
            logger.warning(f"{obs.obs_id}: {remaining_minutes:.1f} min could not be scheduled")
        
        logger.debug(f"    Constraint checker returning {len(sequences)} sequences")

        logger.debug(f"    Final sequences being returned:")
        for idx, (s, e, sci) in enumerate(sequences):
            logger.debug(f"      Seq {idx+1}: start={s}, science={sci:.4f} min")
        return sequences
...
class Task0312Handler:
    ...
    def create_sequences(
        ...
        for idx, (duration_minutes, seq_type) in enumerate(Task0312Handler.SEQUENCE_PATTERN):
    ...
    @staticmethod
    def calculate_scaled_parameters(obs: Observation, scale_factor: float) -> Dict[str, int]:
        """
        ...
        if obs.num_total_frames_requested:
            ...
            if obs.frames_per_coadd and obs.frames_per_coadd > 0:
                scaled_frames = (scaled_frames // obs.frames_per_coadd) * obs.frames_per_coadd
