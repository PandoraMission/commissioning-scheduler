...
class ObservationSequenceData:
    ...
    data_volumes: Dict[str, float] = field(
...
class VisitData:
    ...
    visit_id: str
    observation_sequences: List[ObservationSequenceData] = field(
        default_factory=list
    )
    total_data_volumes: Dict[str, float] = field(
        default_factory=dict
    )  # mode -> total bytes
...
class SOCXMLParser:
    ...
    def parse_all_visits(
        self,
    ) -> Tuple[List[VisitData], List[DataVolumeAlert]]:
    ...
    def _parse_observation_sequence(
        self, seq_elem: ET.Element
    ) -> ObservationSequenceData:
        """
        ...
        seq_data = ObservationSequenceData(
            obs_id=obs_id, sequence_id=obs_id, task_id=task_id, target=target
        )
    ...
    def _calculate_vis_science_volume(self, params: Dict[str, Any]) -> float:
        ...
        num_frames = params.get("NumTotalFramesRequested", 0) / params.get(
            "FramesPerCoadd", 0
        )

        bytes_per_pix = self.BITS_PER_PIX_VIS / 8
        data_vol = (
            roi_dim * roi_dim * max_star_rois * num_frames * bytes_per_pix
        ) * (
            self.COMPRESSION_RATIO_VIS
            * self.OVERHEAD_BUFFER_RATIO
            * self.GENERAL_BUFFER_RATIO
    ...
    def _calculate_nir_volume(self, params: Dict[str, Any]) -> float:
        ...
        data_vol = data_vol_uncompressed * (
            self.COMPRESSION_RATIO_NIR
            * self.OVERHEAD_BUFFER_RATIO
            * self.GENERAL_BUFFER_RATIO
        )
...
def export_to_excel(
    visits: List[VisitData],
    alerts: List[DataVolumeAlert],
    output_file: str = "data_volumes.xlsx",
):
    """
    ...
    for row in ws_seq.iter_rows(
        min_row=2, max_row=ws_seq.max_row, min_col=5, max_col=5
    ):
        for cell in row:
    ...
    # Format data volume column
    for row in ws_visits.iter_rows(
        min_row=2, max_row=ws_visits.max_row, min_col=2, max_col=2
    ):
        for cell in row:
            cell.number_format = "0.000000"
    ...
    for task_id in sorted(
        task_volumes.keys(), key=lambda x: int(x) if x and x.isdigit() else 0
    ):
    ...
    for row in ws_tasks.iter_rows(
        min_row=2, max_row=ws_tasks.max_row, min_col=2, max_col=2
    ):
        for cell in row:
            cell.number_format = "0.000000"
