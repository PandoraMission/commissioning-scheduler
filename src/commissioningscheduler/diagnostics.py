...
from models import (
    Observation, ObservationSequence, Visit,
...
class ScheduleDiagnostics:
    ...
    task_science_durations: Dict[str, float] = field(default_factory=dict)  # task_id -> science minutes only
    task_observation_counts: Dict[str, int] = field(default_factory=dict)  # task_id -> count
    task_sequence_counts: Dict[str, int] = field(default_factory=dict)  # task_id -> count
    
    # Per-observation metrics
    observation_info: Dict[str, Dict] = field(default_factory=dict)  # obs_id -> info dict
    ...
    def has_errors(self) -> bool:
        ...
        return any(hasattr(issue, 'severity') and issue.severity == "error" for issue in self.validation_issues)
    
    @property
    def has_warnings(self) -> bool:
        """Check if there are any warning-level issues."""
        return any(hasattr(issue, 'severity') and issue.severity == "warning" for issue in self.validation_issues)
...
class DiagnosticsAnalyzer:
    ...
    def analyze_schedule(
        ...
        for seq in all_sequences:
            science_minutes = seq.science_duration_minutes if seq.science_duration_minutes is not None else seq.duration_minutes
            overhead_minutes = seq.duration_minutes - science_minutes if science_minutes is not None else 0.0
            
            total_science += science_minutes
            total_overhead += overhead_minutes
        
        scheduling_efficiency = total_science / total_duration if total_duration > 0 else 0.0
        ...
        for seq in all_sequences:
            ...
            if task_id == "CVZ":
                ...
                volume = compute_data_volume_gb(nir_min, vis_min, self.nir_rate, self.vis_rate)
        ...
        task_obs_counts = {task: len(obs_set) for task, obs_set in obs_by_task.items()}
        ...
        if input_observations:
            self._validate_completeness(visits, input_observations, diagnostics)
            self._calculate_visibility_utilization(all_sequences, input_observations, diagnostics)
            self._compare_scheduled_vs_requested(all_sequences, input_observations, diagnostics)
        ...
        logger.info(f"Analysis complete: {len(diagnostics.validation_issues)} issues found")
    ...
    def _compare_scheduled_vs_requested(
        ...
        for obs in input_observations:
            ...
            if difference_seconds < -1.0:
                ...
                diagnostics.validation_issues.append(ValidationIssue(
                    severity="error",
            ...
            if visible_later_obs:
                diagnostics.validation_issues.append(ValidationIssue(
                    ...
                    details={
                        ...
                        'cvz_duration_minutes': (cvz_end - cvz_start).total_seconds() / 60.0,
                        'visible_observations': visible_later_obs[:5],  # Limit to first 5
                        'total_count': len(visible_later_obs)
                    }
                ))

    def _build_observation_info(self, sequences: List[ObservationSequence]) -> Dict[str, Dict]:
        """Build detailed info for each observation."""
        obs_info = defaultdict(lambda: {
            'target': None,
            'ra': None,
            'dec': None,
            'sequence_count': 0,
    ...
    def _build_timeline(self, sequences: List[ObservationSequence]) -> List[Dict]:
    ...
    def _validate_dependencies(
        ...
        for visit in visits:
            ...
            if visit.end_time:
                if visit.visit_id not in task_completion or visit.end_time > task_completion[visit.visit_id]:
                ...
                if prereq not in task_completion:
                    ...
                    diagnostics.validation_issues.append(ValidationIssue(
                        ...
                        details={
                            ...
                            'prerequisite': prereq,
                            'prerequisite_end': task_completion[prereq].isoformat()
                        }
                    ))
    ...
    def _calculate_visibility_utilization(
        ...
        for obs in input_observations:
            for start, end in obs.visibility_windows:
                total_visibility_minutes += (end - start).total_seconds() / 60.0
        ...
        if total_visibility_minutes > 0:
            diagnostics.visibility_utilization = scheduled_minutes / total_visibility_minutes
        else:
    ...
    def _validate_sequence_durations(
        ...
        for seq in sequences:
            ...
            if total_duration > max_duration_minutes:
                ...
                science_minutes = seq.science_duration_minutes if seq.science_duration_minutes is not None else total_duration
                ...
                diagnostics.validation_issues.append(ValidationIssue(
                    ...
                    details={
                        ...
                        'excess_minutes': total_duration - max_duration_minutes
                    }
                ))
    ...
    def _validate_continuous_constraints(
        ...
        for obs_id, obs_sequences in sequences_by_obs.items():
            ...
            if not any(c.applies_to(obs_sequences[0].parent_observation) 
...
def analyze_schedule_from_xml(
    ...
    diagnostics = analyzer.analyze_schedule(visits, input_observations, dependency_json)
...
def _parse_schedule_xml(xml_path: str) -> List[Visit]:
    ...
    for visit_elem in root.findall('Visit'):
        ...
        visit_id = visit_id_elem.text if visit_id_elem is not None else 'Unknown'
        ...
        for seq_elem in visit_elem.findall('Observation_Sequence'):
            ...
            priority = float(priority_elem.text) if priority_elem is not None else 1.0
            
            start_elem = seq_elem.find('.//Timing/Start')
            stop_elem = seq_elem.find('.//Timing/Stop')
            start = parse_utc_time(start_elem.text) if start_elem is not None else datetime.now()
            stop = parse_utc_time(stop_elem.text) if stop_elem is not None else datetime.now()
...
def print_diagnostics_report(
    ...
    # Schedule Overview
    add_line("\nSCHEDULE OVERVIEW:")
    add_line(f"  Start Time:           {diagnostics.schedule_start.strftime('%Y-%m-%d %H:%M:%S')}")
    add_line(f"  End Time:             {diagnostics.schedule_end.strftime('%Y-%m-%d %H:%M:%S')}")
    add_line(f"  Total Duration:       {diagnostics.total_duration_minutes:.1f} minutes ({diagnostics.total_duration_minutes/60:.1f} hours)")
    add_line(f"  Science Time:         {diagnostics.total_science_minutes:.1f} minutes ({diagnostics.total_science_minutes/60:.1f} hours)")
    add_line(f"  Overhead Time:        {diagnostics.total_overhead_minutes:.1f} minutes ({diagnostics.total_overhead_minutes/60:.1f} hours)")
    add_line(f"  Scheduling Efficiency: {diagnostics.scheduling_efficiency:.1%}")
    
    if diagnostics.visibility_utilization > 0:
        add_line(f"  Visibility Utilization: {diagnostics.visibility_utilization:.1%}")
    
    # Data Volume
    add_line(f"\nDATA VOLUME (ESTIMATED):")
    add_line(f"  Total:                {diagnostics.estimated_data_volume_gb:.2f} GB")
    add_line(f"  Budget:               {diagnostics.estimated_data_volume_gb:.2f} / 150.00 GB ({diagnostics.estimated_data_volume_gb/150*100:.1f}%)")
    
    # CVZ Metrics
    add_line(f"\nCVZ IDLE TIME:")
    add_line(f"  Total CVZ Time:       {diagnostics.cvz_time_minutes:.1f} minutes ({diagnostics.cvz_time_minutes/60:.1f} hours)")
    add_line(f"  CVZ Sequences:        {diagnostics.cvz_sequence_count}")
    
    # Task Summary
    add_line(f"\nTASK SUMMARY:")
    add_line(f"  {'Task':<10} {'Observations':<15} {'Sequences':<12} {'Science (min)':<15} {'Total (min)':<15} {'Data (GB)':<10}")
    ...
    for task_id in sorted(diagnostics.task_durations.keys()):
        ...
        science_duration = diagnostics.task_science_durations.get(task_id, 0.0)  # Use science_durations
        total_duration = diagnostics.task_durations[task_id]
        data = diagnostics.data_volume_by_task.get(task_id, 0.0)
        add_line(f"  {task_id:<10} {obs_count:<15} {seq_count:<12} {science_duration:<15.1f} {total_duration:<15.1f} {data:<10.2f}")
    
    # Science time comparison
    science_time_issues = [i for i in diagnostics.validation_issues if i.category == "science_time"]
    if science_time_issues:
        add_line(f"\nSCIENCE TIME COMPARISON (Scheduled vs Requested):")
        add_line(f"  {'Obs ID':<15} {'Requested (s)':<15} {'Scheduled (s)':<15} {'Difference (s)':<15}")
        add_line(f"  {'-'*15} {'-'*15} {'-'*15} {'-'*15}")
        
        for issue in science_time_issues:
            details = issue.details
            add_line(f"  {details['obs_id']:<15} {details['requested_seconds']:<15.1f} {details['scheduled_seconds']:<15.1f} {details['difference_seconds']:<15.1f}")

    # Sequence duration violations
    duration_issues = [i for i in diagnostics.validation_issues if i.category == "sequence_duration"]
    if duration_issues:
        add_line(f"\nSEQUENCE DURATION VIOLATIONS:")
        add_line(f"  Found {len(duration_issues)} sequences exceeding 90-minute limit")
        if detailed:
            add_line(f"  {'Obs ID':<15} {'Seq ID':<10} {'Duration (min)':<15} {'Excess (min)':<15}")
            add_line(f"  {'-'*15} {'-'*10} {'-'*15} {'-'*15}")
            for issue in duration_issues[:20]:  # Show first 20
                details = issue.details
                add_line(f"  {details['obs_id']:<15} {details['sequence_id']:<10} {details['total_duration_minutes']:<15.1f} {details['excess_minutes']:<15.1f}")

    # CVZ gap analysis
    cvz_gap_issues = [i for i in diagnostics.validation_issues if i.category == "cvz_gap"]
    if cvz_gap_issues:
        add_line(f"\nCVZ GAP ANALYSIS:")
        add_line(f"  Found {len(cvz_gap_issues)} CVZ sequences with potentially schedulable observations")
        if detailed:
            for idx, issue in enumerate(cvz_gap_issues[:10], 1):  # Show first 10
                details = issue.details
                add_line(f"\n  CVZ Sequence {idx}:")
                add_line(f"    Time: {details['cvz_start']} to {details['cvz_end']}")
                add_line(f"    Duration: {details['cvz_duration_minutes']:.1f} minutes")
                add_line(f"    Visible later observations ({details['total_count']}): {', '.join(details['visible_observations'])}")

    # Validation Issues
    if diagnostics.validation_issues:
        add_line(f"\nVALIDATION ISSUES ({len(diagnostics.validation_issues)}):")
        
        errors = [i for i in diagnostics.validation_issues if i.severity == "error"]
        warnings = [i for i in diagnostics.validation_issues if i.severity == "warning"]
        infos = [i for i in diagnostics.validation_issues if i.severity == "info"]
    ...
    if detailed and diagnostics.observation_info:
        add_line(f"\nOBSERVATION DETAILS:")
        add_line(f"  {'Obs ID':<15} {'Target':<20} {'Sequences':<10} {'Duration (min)':<15} {'First Start':<20}")
        ...
        for obs_id, info in sorted_obs:
            ...
            first_start = info['first_start'].strftime('%Y-%m-%d %H:%M') if info['first_start'] else 'N/A'
            add_line(f"  {obs_id:<15} {target:<20} {seq_count:<10} {duration:<15.1f} {first_start:<20}")
    
    # Timeline preview
    if detailed and len(diagnostics.timeline) <= 50:
        add_line(f"\nTIMELINE ({len(diagnostics.timeline)} sequences):")
        add_line(f"  {'#':<4} {'Obs ID':<15} {'Target':<15} {'Start':<20} {'Duration':<12}")
        ...
        for i, entry in enumerate(diagnostics.timeline, 1):
            ...
            add_line(f"  {i:<4} {obs_id:<15} {target:<15} {start:<20} {duration:<12}")
...
def analyze_schedule_diagnostics(
    ...
    if sequences is not None:
        ...
        raise ValueError("Must provide sequences, visits, or schedule_xml_path")
    ...
    # Analyze
    analyzer = DiagnosticsAnalyzer(nir_rate, vis_rate, max_data_gb)
    diagnostics = analyzer.analyze_schedule(visits_to_analyze, input_observations, dependency_json)
...
def print_diagnostics_report(
    ...
    if detailed and diagnostics.observation_info:
        add_line(f"\nOBSERVATION DETAILS:")
        add_line(f"  {'Obs ID':<15} {'Target':<20} {'Sequences':<10} {'Science (min)':<15} {'Total (min)':<15}")
        ...
        for obs_id, info in sorted_obs:
            ...
            add_line(f"  {obs_id:<15} {target:<20} {seq_count:<10} {science_duration:<15.1f} {total_duration:<15.1f}")
